...
def loadx(path: Path) -> pd.DataFrame:
    ...
    if "review" not in x.columns:
        log.error(
            f"Table does not contain \"reviews\" [magenta]{path.resolve()}[/]", extra={"markup": True})
        raise ValueError
    log.info(f"X file loaded from [magenta]{path.resolve()}[/]", extra={"markup": True})
    return x
...
def loady(path: Path) -> pd.DataFrame:
    ...
    if "category" not in y.columns:
        log.error(
            f"Table does not contain \"category\" [magenta]{path.resolve()}[/]", extra={"markup": True})
        raise ValueError
    log.info(f"Y file loaded from [magenta]{path.resolve()}[/]", extra={"markup": True})
    return y
...
def loadxy(path: Path) -> pd.DataFrame:
    ...
    if "category" not in xy.columns or "review" not in xy.columns:
        log.error(
            f"Table does not contain \"review\" and \"category\" [magenta]{path.resolve()}[/]", extra={"markup": True})
        raise ValueError
    log.info(f"XY file loaded from [magenta]{path.resolve()}[/]", extra={"markup": True})
...
def ovewrite_guard(func):
    def wrapped(path: Path, *args, **kwargs):
        ...
        if path.exists() and not overwrite:
            log.error(
                f"File already exists [magenta]{path.resolve()}[/]", extra={"markup": True})
...
def dumpx(path: Path, x: pd.DataFrame):
    ...
    log.info(f"Wrote X file to [magenta]{path.resolve()}[/]", extra={"markup": True})


@ovewrite_guard
def dumpy(path: Path, y: pd.DataFrame):
    y.to_csv(path, sep=",", index=True, columns=["category"])
    log.info(f"Wrote Y file to [magenta]{path.resolve()}[/]", extra={"markup": True})
...
def dumpxy(path: Path, xy: pd.DataFrame):
    ...
    log.info(f"Wrote submission file to [magenta]{path.resolve()}[/]", extra={"markup": True})
...
def dump_input(xy: pd.DataFrame, path: Path):
    ...
    fasttext_input(xy).to_csv(
        path, index=False, header=False, quoting=csv.QUOTE_NONE, quotechar="", escapechar="\\"
    )
