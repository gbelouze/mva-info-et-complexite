...
def download(url: str, path: Path):
    log.debug(
        f"Downloading [blue underline]{url}[/] to [magenta]{io.repr(path)}[/]", extra={"markup": True})
        ...
        with open(path, "wb+") as out:
            for chunk in r.iter_content(chunk_size=1024):
                if chunk:
                    out.write(chunk)
    log.info(
        f"Downloaded [blue underline]{url}[/] to [magenta]{io.repr(path)}[/]", extra={"markup": True})
...
class PangLee2005(DatasetDownloader):
    ...
    def download(self):
        if not self.tarfile.is_file():
            ...
            log.debug(
                f"Using cached download of [blue underline]{self.url}[/] at [magenta]{io.repr(self.tarfile)}[/]", extra={"markup": True})
    ...
    def clean(self):
        for file in [self.tarfile, self.csvfile]:
            if file.is_file():
                file.unlink()
                log.info(f"[red]removed[/] [magenta]{io.repr(file)}[/]", extra={"markup": True})

    def describe(self):
        return f"[cyan bold]{len(self):_}[/] Rotten Tomatoes movie reviews annotated with polarity (see http://www.cs.cornell.edu/people/pabo/movie-review-data/ )"
...
class Maas2011(abc.ABC):
    ...
    def __init__(self):
        self.name = f"maas_2005_{self.mode()}"
        self.tarfile = io.download_dir / f"maas_2005.tar.gz"
        self.csvfile = io.xy_dir / f"{self.name}.csv"
    ...
    def describe(self):
        return f"[cyan bold]{len(self):_}[/] Imdb movie reviews annotated with polarity (see [blue underline]https://ai.stanford.edu/~amaas/data/sentiment/[/]) \\[{self.mode()} subset]"
